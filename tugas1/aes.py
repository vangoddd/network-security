...
def inverseMixCol(col):
    ...
    temp.append(mult14[col[0]] ^ mult11[col[1]] ^ mult13[col[2]] ^ mult9[col[3]])
    temp.append(mult9[col[0]] ^ mult14[col[1]] ^ mult11[col[2]] ^ mult13[col[3]])
    temp.append(mult13[col[0]] ^ mult9[col[1]] ^ mult14[col[2]] ^ mult11[col[3]])
    temp.append(mult11[col[0]] ^ mult13[col[1]] ^ mult9[col[2]] ^ mult14[col[3]])
...
def generateSubKey(key):
    expandedKey = textToMat(key)
    #print(expandedKey)

    colIndex = 4
    for i in range(0, 10):
        #Take rightmost col, upward shift
        tempKey = expandedKey[colIndex-1]
        tempKey = shiftUp(tempKey)
        
        for j in range(0, len(tempKey)):
        ...
        #Append 1st col of new key
        expandedKey.append(tempKey)
        #Append the rest of the col
        for j in range(0, 3):
            expandedKey.append(XORcol(expandedKey[colIndex + j], expandedKey[colIndex + j - 3]))
